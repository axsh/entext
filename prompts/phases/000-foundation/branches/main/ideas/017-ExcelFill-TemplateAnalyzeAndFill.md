...
| **参照 Markdown** | 構造 Markdown 以外の補足資料（ルール、用語集、過去成果など）。解析時は意味理解のヒント、埋め込み時は記入内容の根拠として使う |
| **追加プロンプト** | 解析または埋め込み時に付与する自由記述の指示文（ファイルまたは文字列）。テンプレート固有の読み方・記入方針などのヒント |
...
10. **解析ヒント**として、埋め込み時と同様に追加コンテキストを指定できること（いずれも任意、併用可）。
    - `--ref` … 正規表現パターン（既存 `image-to-markdown` の `-r` と同趣旨。複数回指定可）
    - `--ref-dir` … ディレクトリ指定（配下の `.md` を再帰または非再帰で読込。深さは実装計画で定める）
    - `--prompt` … 追加プロンプト文字列（複数回指定可。連結して Vision/意味解析へ付与）
    - `--prompt-file` … 追加プロンプトをファイルから読込（複数回指定可）
    - `--ref` / `--ref-dir` の重複パスは一度だけ読むこと。
    - これらは**系統 1（画像・意味解析）および構造 Markdown 統合**のヒントとし、系統 2 の機械的セル読取結果を改変する正解データにはしないこと。
    - ヒントの利用方針（テンプレートの読み方、記入欄の意味、無視すべき領域など）をプロンプトに明示して付与すること。
...
12. 参照 Markdown および追加プロンプトを複数指定できること（解析側とオプション名を揃える）。
    - `--ref` … 正規表現パターン（複数回指定可）
    - `--ref-dir` … ディレクトリ指定
    - `--prompt` … 追加プロンプト文字列（複数回指定可）
    - `--prompt-file` … 追加プロンプトファイル（複数回指定可）
    - `--ref` / `--ref-dir` の併用を許可し、重複パスは一度だけ読むこと。
13. 対話モードを `--mode json|text` で選択できること（既定は `text`）。
...
14. 対話では、構造 Markdown・参照 Markdown・追加プロンプトを前提に、次を行うこと。
    - 埋めるために必要な情報の聞き取り
    - 不足情報・曖昧な点の確認
    - ユーザー（または連携エージェント）からの回答の収集
15. 収集した情報を元に、テンプレート Excel の対応セルへ値を書き込み、中間 Excel を保存すること。
16. 埋め込み後、**見た目検証ループ**を実行すること。
    1. 埋めた Excel を保存
...
17. リトライ回数を `--max-retries` で指定できること。**既定値は 5**。
...
18. 問題が無い（または許容と判断された）場合、最終版 Excel を `-o` に保存して終了すること（終了コード `0`）。
19. `json` モードでは、質問・不足情報・見た目問題・継続確認・完了通知がすべて JSON メッセージで表現されること。`text` モードでは同等の内容を人間可読テキストで行うこと。
20. LLM / Coding Agent を用いる場合は、既存 `image-to-markdown` と同様に Tern 接続設定（`--server-url` / `--tern-mode` / `--tern-config` / `--agent` / `--model` 等）を引き継げること。

#### D. 非機能・品質

21. テンプレート原本を破壊しないこと。書き込みはコピー上で行い、最終成果のみ `-o` に出すこと。
22. Windows を主対象としつつ、ライブラリ読取/書込パスは可能な限りクロスプラットフォームにすること。見た目検証の PDF 化は既存 backend 選択（`auto` / `excel-com` / `libreoffice`）に従うこと。
23. 単体テストで、セル対応の読取・書込・リトライカウンタ・対話プロトコルのパース・ヒント解決（`--ref` / `--prompt-file`）を LLM なしで検証できること。
24. 統合テストで、フィクスチャ Excel を用いた analyze → fill（モック対話または固定回答）の一連を自動検証できること。
25. `--ref` / `--ref-dir` / `--prompt` / `--prompt-file` の解決ロジックは analyze / fill で**共通パッケージ**に置き、両 CLI で挙動を一致させること。
...
[テンプレート .xlsx]
        │
        ▼
 excel-template-analyze (+ --ref/--ref-dir/--prompt/--prompt-file)
   ├─ (画像) excel-to-pdf → pdf-to-image → Vision 意味解析（ヒント付与）
  ...
  構造 Markdown (.md)
        │
        ▼
 excel-fill (--template, --structure, --ref/--ref-dir/--prompt/--prompt-file, --mode, --max-retries)
...
bin/entext/excel-template-analyze \
  ...
  --ref "docs/template-hints/.*\\.md" \
  --ref-dir docs/glossary \
  --prompt "記入欄は薄い黄のセルのみ。グレー帯は固定見出し。" \
  --prompt-file docs/hints/analyze_extra.txt \
...
bin/entext/excel-fill \
  ...
  --ref "docs/rules/.*\\.md" \
  --ref-dir docs/glossary \
  --prompt "数値は半角、日付は YYYY-MM-DD。" \
  --prompt-file docs/hints/fill_extra.txt \
  --mode text \
  --max-retries 5 \
...
| 参照・追加プロンプト解決 | `internal/common/refprompt/` 等（`--ref` / `--ref-dir` / `--prompt` / `--prompt-file` を analyze/fill で共有） |
| Excel セル I/O | `internal/excelcell/`（読取/書込の薄いラッパ。ライブラリ依存をここに閉じる） |
| CLI | `cmd/excel-template-analyze/`, `cmd/excel-fill/` |
| 公開 API | `entext.go` に `AnalyzeExcelTemplate` / `FillExcel`（名称は実装計画で確定）。両 API に Hint/Ref/Prompt オプションを持たせる |

既存の `internal/exceltopdf` / `internal/pdftoimage` / `internal/imagetomd` / `internal/exceltocsv` は**呼び出し側から再利用**し、変換ロジックの複製は避ける。

### 4. 構造 Markdown とヒントの役割

- 編集エージェント（または fill 内部の LLM）が「どこに何を書くか」を決めるための**唯一の主地図**とする。
- 画像由来の意味記述と、ライブラリ由来のセル番地を同一ドキュメント内で対応づける。
- 参照 Markdown（`--ref` / `--ref-dir`）と追加プロンプト（`--prompt` / `--prompt-file`）は補足コンテキストである。
  - **analyze**: 意味構造の読み取りヒント（どの色が入力欄か、シートの読み順など）。セル座標の機械読取はライブラリ結果を正とする。
  - **fill**: 記入内容・体裁のヒント。セル座標の正本は構造 Markdown とする。
- ヒント解決は共通化し、オプション名・重複排除・読込失敗時のエラー（exit 2）を両 CLI で揃える。
...
4. **ヒント指定（ref / prompt）は analyze と fill で同型**とし、共通解決層に寄せる。
5. 既存変換ツールの I/O 契約・exit code を踏襲し、 entext 公開 API から一貫利用可能にする。
...
2. **解析時のヒント指定**
   1. 「薄い黄セルだけが記入欄」などと書いた `--prompt-file` と、関連 `--ref` Markdown を用意する。
   2. `excel-template-analyze` に `--ref` / `--ref-dir` / `--prompt` / `--prompt-file` を付けて実行する。
   3. 構造 Markdown の意味構造がヒントに沿って解釈され（記入欄の取りこぼしや誤認が減る方向）、セル番地はライブラリ結果と対応していることを確認する。
3. **参照・プロンプトの一括読込（fill）**
   1. 複数の参照 Markdown と追加プロンプトをディレクトリ・正規表現・ファイルで用意する。
   2. `excel-fill` に `--ref` / `--ref-dir` / `--prompt` / `--prompt-file` を同時指定する。
...
| B/C. 参照・追加プロンプト（analyze/fill 共通） | 単体: `--ref` / `--ref-dir` / `--prompt` / `--prompt-file` の解決・重複排除・欠落ファイルで exit 2。プロンプト連結順の固定 |
