...
func main() {
	...
	cmd := &cobra.Command{
		Use:   "excel-to-pdf",
		Short: "Convert Excel files into PDF",
		Run: func(cmd *cobra.Command, args []string) {
			v, err := config.SetupViper(cmd, configPath, "doc_convert_excel_pdf")
			if err != nil {
			...
			for _, input := range inputs {
				logger.Debug("converting excel file", "input", input, "output_dir", outputDir, "backend", backend, "engine", engine, "sheets", sheets)
				artifact, convErr := entext.ConvertExcelToPDFWithOptions(ctx, entext.FileJob{
					InputPath: input,
					OutputDir: outputDir,
