...
- Phase 2 で読み取った**画像可視スコープ内**の行データ（R01, R02, ...等）が最終出力に全て含まれているか自己チェックしてください。
...
6. 言語は日本語で出力する。
7. Phase 2 で CSV 参照により取得した行のうち、画像可視スコープ外の行は最終出力から除外すること。
8. 可視スコープは Phase 1 で確定した行（known_info サマリ参照）を正とする。
9. Phase 2 answer に No.1 等の過剰行があっても、最終 MD には可視行のみを出力すること。
